--[[
	shit ass code.
	made by nobius / vk.com/neverborn
--]]

ui.add_checkbox( "enable", "grenade_esp", false )
ui.add_checkbox( "hegrenade", "grenade_esp_he", false )
ui.add_slider_int( "max distance", "he_dist", 1, 500, 1 )
ui.add_checkbox( "molotov", "grenade_esp_molotov", false )
ui.add_slider_int( "max distance ", "molotov_dist", 1, 500, 1 )
ui.add_checkbox( "burning molotov", "grenade_esp_burn_molotov", false )
ui.add_slider_int( "max distance  ", "burn_dist", 1, 500, 1 )
...
local function call_molotov_fire( pos )
	local get_int = ui.get_int( "burn_dist" )
	local eye_pos = pos
	
	local grenades = entitylist.get_entities_by_class_id( 100 )
	...
	if grenades then
		for i = 1, #grenades do
			...
			if grenade then
				...
				local safe = dist > 15 or fraction < 0.61
...
local function call_molotov( pos )
	local get_int = ui.get_int( "molotov_dist" )
	local burn = ui.get_bool( "grenade_esp_burn_molotov" )
	
	local eye_pos = pos
	
	local grenades = entitylist.get_entities_by_class_id( 113 )
	local correct_shit = 0
	...
	if grenades then
		for i = 1, #grenades do
			...
			if grenade then
				...
				local dist = distance_in_ft( eye_pos, ent_origin )
				local safe = dist > 20 or fraction < 0.61
...
local function call_he( pos )
	local get_int = ui.get_int( "he_dist" )
	local eye_pos = pos
...
local function render( )
	...
	local start = ui.get_bool( "grenade_esp" )
	
	local he = ui.get_bool( "grenade_esp_he" )
	local molotov = ui.get_bool( "grenade_esp_molotov" )
	local burn = ui.get_bool( "grenade_esp_burn_molotov" )
...
client.register_callback( "paint_d3d", render )